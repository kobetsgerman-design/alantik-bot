...
# Автопостинг: время и дни недели (0=понедельник ... 6=воскресенье)
MOSCOW_TZ = timezone(timedelta(hours=3))
AUTO_POST_TIME = dtime(hour=18, minute=0, tzinfo=MOSCOW_TZ)
AUTO_POST_DAYS = (0, 1, 2, 3, 4)  # будни: пн-пт
...
#антиквариат #мебель #винтаж #люстра #интерьер
    ...
    async def start_handler(self, update, context):
        ...
        await context.bot.send_message(
            ...
            text=(
                "🤖 Alantik Bot is running!\n\n"
                "⏰ Автопостинг: каждый будний день в 18:00 (МСК)\n"
                "📤 /post — опубликовать новые товары прямо сейчас (вручную)"
            )
        )
    ...
    async def scheduled_post_job(self, context: ContextTypes.DEFAULT_TYPE):
        """Called automatically by the job queue on the configured schedule"""
        logger.info("⏰ Автопостинг по расписанию запущен")
        await self.post_all_products()
    ...
    def setup_schedule(self):
        """Schedule automatic daily posting"""
        if self.application.job_queue is None:
            logger.warning(
                "⚠️ JobQueue недоступен — автопостинг не будет работать. "
                "Убедитесь что установлен пакет python-telegram-bot[job-queue]"
            )
            return
        self.application.job_queue.run_daily(
            ...
            time=AUTO_POST_TIME,
            days=AUTO_POST_DAYS,
            name="daily_auto_post",
        )
        logger.info(
            f"⏰ Автопостинг запланирован на {AUTO_POST_TIME.strftime('%H:%M')} (МСК), будни"
        )
